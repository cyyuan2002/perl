...
while(<$fh_filelst>){
    ...
    while(<$fh_brkfile>){
        ...
        if($lines[5] eq "pass"){
            ...
            if($lastchrom ne $lines[0]){
                if($lastchrom ne ""){
                    $chrsvcount{$lastchrom}=$svcount;
                    $chrsvlength{$lastchrom}=$svlength;
                }
                $svlength=0;
                $svcount=0;
                $lastchrom=$lines[0];
            ...
            $svcount++;
            $svlength+=abs($lines[3]-$lines[1]+1);
        }
    }
    close $fh_brkfile;
    $chrsvcount{$lastchrom}=$svcount;
    ...
    for(my $i=1;$i<=$chromcount;$i++){
        ...
        if(exists($chrsvcount{$chromName})){
            print $fh_strainout "\t$chrsvcount{$chromName}\t$chrsvlength{$chromName}";
        }
        else{
            print $fh_strainout "\t0\t0";
        }
    }
    print $fh_strainout "\n";
}
...
open(my $fh_rgn,">$OutFile.rgn");
foreach my $chrom (sort keys %allbreaks){
    my @brks=@{$allbreaks{$chrom}};
    my $SVcount;
    my $SVlength;
    for(my $i=0;$i<@brks;$i++){
        print $fh_rgn "$chrom\t",$brks[$i]->{'posA'},"\t","$chrom\t",$brks[$i]->{'posB'},"\t",$brks[$i]->{'sv'},"\t",$brks[$i]->{'count'},"\n";
        $SVcount++;
        $SVlength+=$brks[$i]->{'posB'}-$brks[$i]->{'posA'} +1;
    }
    print $fh_out "$chrom\t$SVcount\t$SVlength\n";
}
close $fh_out;
close $fh_rgn;
