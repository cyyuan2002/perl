...
if (defined $opts{f1}) {
	...
	$count=0;
	$file=1;
	open (OUT,">$out_dir/$name.f1.$file.seq");
	while (<IN>) {
		chomp;
		if ($_ ne "") {
			print OUT ">$_";
	...
	close OUT ;
	if ($count==0) {
		system ("rm -rf $out_dir/$name.f1.$file.seq")
	}
	close IN;
}
...
if (defined $opts{f2}) {
	...
	if ($in_seq_num < $file_num) {
		die "There are only $name_seq_num sequences in $in .\n";
	}
	...
	$count=0;
	$file=1;
	print "$in_seq_num\t$seq_num\t$limit\t$file\n";
	open (OUT,">$out_dir/$name.f2.$file.seq");
	while (<IN>) {
		chomp;
		if ($_ ne "") {
			print OUT ">$_";
	...
	close OUT ;
	if ($count==0) {
		system ("rm -rf $out_dir/$name.f2.$file.seq")
	}
	close IN;
}
	...
	while (<IN>) {
		...
		if ($_ ne "") {
			...
			while (length($seq) > $size) {
				$start=$count*$cut+1;
				$end=$start+$size-1;
				print OUT ">$nam\_$start-$end   Length:$size\n";
			...
			$start=$count*$cut+1;
			$end=$start+$size-1;
			print OUT ">$nam\_$start-$end   Length:$end_size\n";
			$seq=~s/(.{50})/$1\n/g;
			$seq=~s/\n$//;
			print OUT "$seq\n";
		}
