...
use Getopt::Long;
my %opts;
my ($List,$Len_th,$is_detail,$GC_th,$Help);
GetOptions(\%opts,"l:s"=>\$List,"d:i"=>\$is_detail,"s:s"=>\$Len_th,"gc:s"=>\$GC_th,"help"=>\$Help);
if ((defined $Help)||(@ARGV==0))
{
	die "perl seq_N50.pl <IN:fasta|fastq> [-d showdetails default:0][-l calculate-contig-list] [s: length-theshold] [gc: GC-content-theshold] [-help]
Example: perl seq_N50.pl contigs.fasta -d 1 -l contigs.list -s 100 -gc 0.2-0.6\n
Note:
